...
function isTabOutUrl(url, tabOutUrl) {
...
async function fetchOpenTabs() {
  try {
    ...
    openTabs = tabs.map(t => ({
      ...
      windowId: t.windowId,
      active:   t.active,
      isTabOut: isTabOutUrl(t.url || '', tabOutUrl),
    }));
  } catch {
 ...
 * closeTabOutDupes()
 *
 * Closes duplicate Tab Out dashboard pages in the current window.
 */
async function closeTabOutDupes() {
  ...
  const tabOutTabs = allTabs.filter(t =>
    t.windowId === currentWindow.id && isTabOutUrl(t.url || '', tabOutUrl)
  );
 ...
 * checkTabOutDupes()
 *
 * Counts Tab Out pages in this window. If more than 1, shows a banner
 * offering to close the extras.
 */
function checkTabOutDupes() {
  const tabOutTabs = openTabs.filter(t => t.isTabOut && t.windowId === currentWindowId);
